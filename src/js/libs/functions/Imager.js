/**
 * Usage:
 * const Imager = require('./libs/functions/Imager');
 * 
 * @todo
 * 
 */
...
var Imager = function (elements, opts) {
    ...
    this.screenDirection = '-wide';
    this.useOrientation = opts.hasOwnProperty('useOrientation') ? opts.useOrientation : true;
    this.useSize = opts.hasOwnProperty('useSize') ? opts.useSize : true;
...
Imager.prototype.add = function (elementsOrSelector) {
    ...
    if (elements && elements.length) {
        ...
        elements = this.changeDivsToEmptyImages(additional);
...
Imager.prototype.createGif = function (element) {
    ...
    for(;i<l;i++) {
        gif.setAttribute(element.attributes[i].name, element.attributes[i].value);
    }
    
    gif.className = (elementClassName ? elementClassName + ' ' : '') + this.className +  ' ' + element.className;
...
Imager.prototype.replaceImagesBasedOnScreenDimensions = function (image) {
    ...
    if (!this.needDirectionUpdate(image) && !this.isPlaceholder(image) && computedWidth <= naturalWidth) {
        return;
    }

    image.src = this.changeImageSrcToUseNewImageDimensions(image.getAttribute('data-src'), computedWidth, image);
    image.removeAttribute('width');
    image.removeAttribute('height');
};

Imager.prototype.determineAppropriateResolution = function (image) {
    return Imager.getClosestValue(image.getAttribute('data-width') || image.parentNode.clientWidth || window.innerWidth, this.availableWidths);
};
 ...
 * Addons
 */
Imager.prototype.getScreenDirection = function () {
    return this.screenDirection;
}

Imager.prototype.updateScreenDirection = function() {
    let mql = window.matchMedia("(orientation: portrait)");
    this.screenDirection = mql.matches ? '-tall' : '-wide';

    return this.screenDirection;
}

Imager.prototype.needDirectionUpdate = function(image) {
    const oldDir = image.getAttribute('data-orientation');
    const newDir = this.updateScreenDirection();
 ...
 * End Addons
 */
Imager.prototype.changeImageSrcToUseNewImageDimensions = function (src, selectedWidth, img) {
    const direction = this.useOrientation ? this.updateScreenDirection() : '-wide';
    const size = this.useSize ? (window.innerWidth < 640 ? '-small' : '-big') : '-big';

    img.setAttribute('data-orientation', direction);
    return src
...
Imager.transforms = {
    ...
    width: function (width, map) {
        return map[width] == undefined ? width : map[width];
    }
};
