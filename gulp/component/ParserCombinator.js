...
const notStr = (s) => new Parser((state) => {
    const {
        target,
        index,
        isError
    ...
    return updateStateError(state, `str: Tried to not match "${s}", but got "${target.slice(index, index + 10)}"`);
})
...
const digits = () => new Parser((state) => {
    ...
    return updateStateError(state, `digits: Tried to match "0-9", but got "${target.slice(index, index + 10)}"`);
});
...
const many1 = parser => new Parser(state => {
    ...
    if (results.length === 0) {
        return updateStateError(
            state,
            `many1: Unable to match any input using ${parser.constructor.name} @ index ${state.index}: ${state.target.slice(state.index, state.index + 10)}`
        );
    }
    return updateStateResult(next, results);
});
...
const anyOf = chars => new Parser(state => {
    ...
    if (chars.indexOf(target[index]) > -1) {
